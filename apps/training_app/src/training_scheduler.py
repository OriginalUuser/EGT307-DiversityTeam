...
async def run_training_batch(config: TrainingConfig):
    ...
    logger.info(f"Queuing Batch {batch_id}: {total_jobs} total tasks ({len(target_tables)} tables x {len(target_columns)} columns)")
    ...
    return {"batch_id": batch_id, "total_jobs": len(target_tables) * len(target_columns)}
