...
class TrainingConfig(BaseModel):
    ...
    tables: Optional[List[str]] = Field(None, description="List of tables to retrain. If null, all tables are used.")
    columns: Optional[List[str]] = Field(None, description="Specific features to retrain. If null, all features used.")
    rows_limit: Optional[int] = Field(5000, description="Number of rows to use for training.")

    # LSTM Hyperparameters
    window_size: int = Field(10, description="Number of past time steps to look back (sliding window).")
